function [RESULTS_GLOBAL,Ifin2] = imagecompression_color_other_YCbCr(filepath)
...
for kstep = 5:Kmax
	...
	t1 = toc;
	tic;
        num_keep = keep*3*Nor*Mor;
        keep_Y = 0.60*num_keep / length(IdecR);
        keep_C = 0.20*num_keep / length(IdecR);
        switch threshold_type
            case 1
                [IcompR, zero_elR] = THR_other(IdecR, SdecR, keep_Y,flag);
                [IcompG, zero_elG] = THR_other(IdecG, SdecG, keep_C,flag);
                [IcompB, zero_elB] = THR_other(IdecB, SdecB, keep_C,flag);
