function [RESULTS_GLOBAL] = imagecompression_color_OW_new(filepath)
...
for kstep = 3:Kmax
    for keep = vkeep
        ...
        [IdecR, IscalR, IwavR, lrow, lcol] = DEC_new(R, theta, kstep, dmin,...
            initial_transformation);
        [IdecG, IscalG, IwavG, lrow, lcol] = DEC_new(G, theta, kstep, dmin,...
            initial_transformation);
        [IdecB, IscalB, IwavB, lrow, lcol] = DEC_new(B, theta, kstep, dmin,...
            initial_transformation);
        t1 = toc;
        tic;
        switch threshold_type
            case 1
                [thr, zero_elR, IcompR] = THR(IdecR, IscalR, IwavR, keep, lrow, lcol,...
                    flag);
                [thr, zero_elG, IcompG] = THR(IdecG, IscalG, IwavG, keep, lrow, lcol,...
                    flag);
                [thr, zero_elB, IcompB] = THR(IdecB, IscalB, IwavB, keep, lrow, lcol,...
                    flag);
        ...
        t2 = toc;
        tic;
        IrecR = REC_new(IcompR, lrow, lcol, theta, initial_transformation);
        IrecG = REC_new(IcompG, lrow, lcol, theta, initial_transformation);
        IrecB = REC_new(IcompB, lrow, lcol, theta, initial_transformation);
