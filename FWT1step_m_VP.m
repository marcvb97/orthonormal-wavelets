...
% Initialization
%------------------
%%%%%% Definition of n and N=3n
N1 = length(a);
r1 = rem(N1, 3);
if r1 == 0
    N     = N1;  n = N/3;
    nplus = 0;
else  % enlarge a (of 1 or 2 elem.) by replicating the last entry
    nplus = 3 - r1;
    for k = 1:nplus
        a(N1+k) = a(N1);
    end
...
% FIX: when m >= n the overlap loop would be vacuous; clamp m to n-1
% This can happen for very short fibers (small n) combined with large theta.
if m >= n
    m = n - 1;
end
if m == 0   % guard again after clamping (n=1 edge case)
    m = 1;
end
...
alphaf = dct(ap * pi/N) * sqrt(n/pi);       % length n

% STEP 2:
app = a;
app(2:3:end) = 0;                           % length N=3n, n entries are zero
y2  = dct(app * pi/N) * sqrt(N/pi);        % length N

% Compute betaf — must reach length n
% Initialise with the first n-m entries (no mixing needed there)
betaf = y2(1:n);                            % FIX: preallocate to length n
for k = n-m+1 : n-1
...
ab = (alphaf + betaf) ./ mu;               % both length n — no size mismatch
