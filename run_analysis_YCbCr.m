...
function run_analysis_YCbCr(dataset_name)
...
saveas(1, sprintf('figs/fig_%s_PSNR_YCbCr.eps', dataset_name), 'epsc')
...
legend("OVP","OVP2","IVP","DB2","BIOR3.5","Location","northwest",'Interpreter', 'latex')
saveas(2, sprintf('figs/fig_%s_SSIM_YCbCr.eps', dataset_name), 'epsc')
...
legend("OVP","OVP2","IVP","DB2","BIOR3.5","Location","northwest",'Interpreter', 'latex')
saveas(3, sprintf('figs/fig_%s_level_YCbCr.eps', dataset_name), 'epsc')
...
saveas(4, sprintf('figs/fig_%s_theta_YCbCr.eps', dataset_name), 'epsc')
...
function filename = get_filename(dataset_name, suffix)
    ...
    if ismember(dataset_name, par_datasets) && ismember(suffix, par_suffixes)
        filename = sprintf('mat/%s_%s_YCbCr_par.mat', dataset_name, suffix);
    else
        filename = sprintf('mat/%s_%s_YCbCr.mat', dataset_name, suffix);
    end
end