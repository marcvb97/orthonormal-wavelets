function [RESULTS_GLOBAL] = imagecompression_color_OW_new_par(filepath)

% AIM: Image compression using OVP wavelets.
...
for kstep = 3:Kmax
    for keep = vkeep
        ...
        parfor ch = 1:3
            ...
            tic;
            [Idec, Iscal, Iwav, lrow, lcol] = DEC_new(Chan, theta, kstep, dmin, ...
            ...
            tic;
            Irec   = REC_new(Icomp, lrow, lcol, theta, initial_transformation);
        ...
        if threshold_type == 2
            [IdecR, IscalR, IwavR, lrow, lcol] = DEC_new(channels{1}, theta, kstep, dmin, initial_transformation);
            [IdecG, IscalG, IwavG, ~,    ~   ] = DEC_new(channels{2}, theta, kstep, dmin, initial_transformation);
            [IdecB, IscalB, IwavB, ~,    ~   ] = DEC_new(channels{3}, theta, kstep, dmin, initial_transformation);
            [~, zero_el, IcompR, IcompG, IcompB] = THR_color(IdecR, IdecG, IdecB, ...
                IscalR, IscalG, IscalB, IwavR, IwavG, IwavB, keep, lrow, lcol, flag);
            Iout{1} = REC_new(IcompR, lrow, lcol, theta, initial_transformation); Iout{1} = Iout{1}(1:Nor,1:Mor);
            Iout{2} = REC_new(IcompG, lrow, lcol, theta, initial_transformation); Iout{2} = Iout{2}(1:Nor,1:Mor);
            Iout{3} = REC_new(IcompB, lrow, lcol, theta, initial_transformation); Iout{3} = Iout{3}(1:Nor,1:Mor);
        ...
        RESULTS(cont, :) = [keep, psnrVAL, SSIMVAL, ...
            (num - zero_el)*100/(3*Nor*Mor), theta_fin, kmax]
