function [RESULTS_GLOBAL] = imagecompression_color_OW_par(filepath)
...
for kstep = 4:Kmax
    for keep = vkeep
        ...
        lrow_arr = zeros(1,3);
        lcol_arr = zeros(1,3);
        IscalLen = zeros(1,3);  % to compute `num` later
        ...
        parfor ch = 1:3
            ...
            if initial_transformation
                AChan = TR2D(Chan, theta);
            else
                AChan = Chan;
            end
            [Idec, Iscal, Iwav, lrow, lcol] = DEC(AChan, theta, kstep, dmin);
            t1_arr(ch) = toc;
        ...
        % threshold_type == 2 (combined) cannot be trivially parallelized per
        % channel; fall back to sequential in that case.
        if threshold_type == 2
            [~, zero_el, Iout{1}, Iout{2}, Iout{3}] = THR_color( ...
                DEC(channels{1},theta,kstep,dmin), ...   % re-run is avoided by
                DEC(channels{2},theta,kstep,dmin), ...   % storing Idec/Iscal/Iwav
                DEC(channels{3},theta,kstep,dmin), ...   % if perf matters; kept
                keep, lrow_arr(1), lcol_arr(1), flag);   % simple here for clarity
            zero_els = [zero_el/3, zero_el/3, zero_el/3]; % split equally for num
        end
