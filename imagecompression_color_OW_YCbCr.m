function [RESULTS_GLOBAL,Ifin2] = imagecompression_color_OW_YCbCr(filepath)
...
for kstep = 4:Kmax
    for keep = vkeep
        ...
        [IdecR, IscalR, IwavR, lrow, lcol] = DEC(AR, theta, kstep, dmin);
        [IdecG, IscalG, IwavG, lrow, lcol] = DEC(AG, theta, kstep, dmin);
        [IdecB, IscalB, IwavB, lrow, lcol] = DEC(AB, theta, kstep, dmin);
        t1 = toc;
        tic;
        ...
        t2 = toc;
        tic;
        IrecR = IFWTmatrix_m(IcompR, lrow, lcol, theta);
        IrecG = IFWTmatrix_m(IcompG, lrow, lcol, theta);
        IrecB = IFWTmatrix_m(IcompB, lrow, lcol, theta);

        IoutR = IrecR(1:Nor, 1:Mor);
        IoutG = IrecG(1:Nor, 1:Mor);
        IoutB = IrecB(1:Nor, 1:Mor);
        if initial_transformation
            IoutR = ITR2D(IoutR,theta);
            IoutG = ITR2D(IoutG,theta);
            IoutB = ITR2D(IoutB,theta);
        end
        t3 = toc;
